...
def debug_partial_transparency():
    """部分的透明度問題の詳細調査"""
    print("=== 部分的透明度問題の詳細調査 ===")
    ...
    # 4. 関節リンクの色設定テスト
    print("\n--- 関節リンクの色設定テスト ---")
    for i in range(min(4, num_joints)):  # 最初の4関節のみテスト
        print(f"  関節 {i} の色設定テスト...")
        ...
        colors = [
            ...
            [0.5, 0.0, 1.0, 1.0],  # 紫
            [0.0, 1.0, 1.0, 1.0],  # シアン
            [1.0, 0.0, 1.0, 1.0],  # マゼンタ
        ]
        
        if i < len(colors):
            p.changeVisualShape(robot_id, i, rgbaColor=colors[i])
            ...
            if visual_data:
                ...
                color_diff = np.linalg.norm(np.array(current_color[:3]) - np.array(colors[i][:3]))
                if color_diff < 0.1:
                    print(f"    ✅ 関節 {i} 色設定成功")
                else:
                    print(f"    ❌ 関節 {i} 色設定失敗 (差分: {color_diff:.3f})")
    ...
    # 現在のレンダリング設定
    print("  現在のレンダリング設定:")
    print(f"    レンダラー: {p.ER_TINY_RENDERER}")
    ...
    try:
        ...
        for pos, name in camera_positions:
            print(f"\n  {name}カメラ ({pos}):")
            ...
            view_matrix = p.computeViewMatrixFromYawPitchRoll(
                ...
                yaw=np.degrees(np.arctan2(pos[1], pos[0])),
                pitch=np.degrees(np.arctan2(-pos[2], np.sqrt(pos[0]**2 + pos[1]**2))),
                roll=0,
                upAxisIndex=2
            )
    ...
    print("2. 色設定の段階的テスト")
    print("3. 複数カメラ角度での画像分析")
    print("4. 物理パラメータの確認")


if __name__ == "__main__":
    debug_partial_transparency()
